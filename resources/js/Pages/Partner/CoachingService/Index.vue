        ...
        <section class="rounded-md bg-white p-6 shadow-sm shadow-sky-950/5">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    ...
                    <h1 class="mt-3 text-3xl font-black tracking-tight">Layanan Coaching</h1>
                    <p class="mt-3 text-sm text-slate-500">Kelola layanan coaching Anda.</p>
                </div>
            </div>
        </section>
        ...
        <section class="mt-5 rounded-md bg-white p-5 shadow-sm shadow-sky-950/5">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    <h2 class="text-xl font-black">Daftar Layanan Saya</h2>
                        ...
                        <tr v-for="service in services.data" :key="service.id" class="border-t border-slate-100">
                            ...
                            <td class="px-4 py-4">
                                <span class="rounded px-3 py-1 text-xs font-bold" 
                                        ...
                                        'bg-sky-100 text-sky-800': service.status === 'pending',
                                        'bg-red-100 text-red-800': service.status === 'rejected',
                                        'bg-gray-100 text-gray-800': service.status === 'archived'
                                        }">
                                    {{ service.status }}
                                </span>
                                <div v-if="service.status === 'rejected' && service.rejection_reason" class="mt-2 text-xs text-red-600">
                                    <span class="font-bold">Alasan Ditolak:</span> {{ service.rejection_reason }}
                                </div>
                            </td>
                            <td class="px-4 py-4">
