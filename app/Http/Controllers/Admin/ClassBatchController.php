...
class ClassBatchController extends Controller
{
    ...
    public function index(Product $product)
    {
        ...
        $batches->each(function (ClassBatch $batch) use ($product): void {
            $batch->setAttribute(
                'update_url',
                route('admin.class-products.batches.update', [
                    'product' => $product->id,
                    'batch' => $batch->id,
                ], false)
            );
            $batch->setAttribute(
                'delete_url',
                route('admin.class-products.batches.destroy', [
                    'product' => $product->id,
                    'batch' => $batch->id,
                ], false)
            );
        ...
        return Frontend::render('Admin/ClassProducts/Batches', [
            'product' => $product,
            'batches' => $batches,
            'storeUrl' => route('admin.class-products.batches.store', ['product' => $product->id], false),
            'statuses' => [
    ...
    {
        $this->ensureBatchOwner($product, $batch);
        $data = $this->validated($request);
    ...
    public function destroy(Product $product, ClassBatch $batch): RedirectResponse
    {
        $this->ensureBatchOwner($product, $batch);
    ...
    {
        return $this->destroy($product, $this->batchFromRequest($request, $product));
    }
