...
const applyFilter = () => {
    router.get('/admin/class-approvals', {
...
const approveProduct = async () => {
    const confirmed = await confirmAction({
        title: 'Setujui kelas?',
        text: `kelas "${selected.value.name}" akan diterbitkan.`,
        confirmButtonText: 'Ya, setujui',
    });

    if (!confirmed) return;

    router.patch(`/admin/class-approvals/${selected.value.id}/approve`, {}, {
        preserveScroll: true,
        onSuccess: () => {
            showModal.value = false;
            successAlert('kelas berhasil disetujui dan diterbitkan.');
        },
        onError: () => errorAlert('kelas gagal disetujui.'),
    });
};
...
const rejectProduct = async () => {
    const confirmed = await confirmAction({
        title: 'Tolak kelas?',
        text: `kelas "${selected.value.name}" akan ditolak dengan alasan yang diisi.`,
    ...
    router.patch(`/admin/class-approvals/${selected.value.id}/reject`, {
        rejection_reason: rejectionReason.value,
    }, {
        preserveScroll: true,
        onSuccess: () => {
            showModal.value = false;
            successAlert('kelas berhasil ditolak.');
        },
        onError: () => errorAlert(),
    });
};
    ...
    <AdminLayout>
        <section class="rounded-md bg-white p-6 shadow-sm shadow-sky-950/5">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    ...
                    <h1 class="mt-3 text-3xl font-black tracking-tight">Approval Kelas</h1>
                    <p class="mt-3 text-sm text-slate-500">Tinjau kelas baru atau pembaruan yang diajukan oleh Vendor.</p>
                </div>
        ...
        <section class="mt-5 rounded-md bg-white p-5 shadow-sm shadow-sky-950/5">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <h2 class="text-xl font-black">Daftar kelas Pending</h2>
                <div class="flex gap-2 w-full md:w-auto">
                    <input v-model="filter.search" @keyup.enter="applyFilter" class="w-full md:w-64 rounded-md border border-slate-200 px-3 py-2 text-sm" placeholder="Cari nama kelas...">
                ...
                <table class="w-full min-w-[980px] text-left text-sm">
                    ...
                    <tbody>
                            ...
                            <td class="px-4 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-bold">
                                            {{ product.name }}
                                            <span v-if="isVendorUpdate(product)" class="ml-2 inline-flex items-center gap-1 rounded bg-amber-100 px-2 py-0.5 text-[10px] font-black uppercase text-amber-700" title="Vendor telah mengubah data kelas ini. Silakan tinjau kembali.">Update Vendor</span>
                        ...
                        <tr v-if="products.data.length === 0">
                            <td colspan="5" class="px-4 py-10 text-center text-slate-500">Tidak ada kelas yang menunggu persetujuan.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div v-if="showModal && selected" class="fixed inset-0 z-50 grid place-items-start bg-slate-950/45 px-4 py-10 backdrop-blur-sm overflow-y-auto">
            <div class="mx-auto w-full max-w-3xl overflow-hidden rounded-md bg-white shadow-2xl shadow-slate-950/20 my-auto">
                <div class="flex items-start justify-between border-b border-slate-100 px-6 py-5 bg-white">
                    <div>
                        <h2 class="text-xl font-black">Detail kelas</h2>
                ...
                <div class="max-h-[60vh] overflow-y-auto px-6 py-6 space-y-6">
                    ...
                    <div v-if="isRejecting" class="p-4 bg-red-50 border border-red-200 rounded-lg">
                        <form @submit.prevent="rejectProduct">
                            ...
                            <textarea name="rejection_reason" required v-model="rejectionReason" class="mt-2 w-full rounded-md border border-red-200 px-3 py-2 text-sm focus:border-red-500 focus:ring-red-500" rows="3" placeholder="Jelaskan alasan kelas ini ditolak..."></textarea>
                ...
                <div v-if="!isRejecting" class="flex justify-end gap-3 border-t border-slate-100 px-6 py-4 bg-slate-50">
                    <button type="button" @click="isRejecting = true" class="rounded-md border border-red-200 bg-white px-5 py-2.5 text-sm font-black text-red-600 hover:bg-red-50">Tolak kelas</button>
