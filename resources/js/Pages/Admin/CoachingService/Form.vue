...
const form = useForm({
    ...
    mentor_id: props.service?.mentor_id || '',
        ...
        <section class="rounded-md bg-white p-6 shadow-sm shadow-sky-950/5">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    ...
                    <p class="mt-3 text-sm text-slate-500">Lengkapi form di bawah ini.</p>
                </div>
            </div>
        </section>
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div class="md:col-span-2">
                        <label class="text-sm font-bold">Thumbnail
                            ...
                            <div v-if="form.errors.thumbnail" class="mt-1 text-xs text-red-500">{{ form.errors.thumbnail }}</div>
                        </label>
                    </div>
                    ...
                    <!-- Mentor -->
                    <div>
                        <label class="text-sm font-bold">Mentor
                            <select v-model="form.mentor_id" class="mt-2 w-full rounded-md border border-slate-200 px-4 py-3">
                                <option value="">Pilih Mentor</option>
                                <option v-for="mentor in mentors" :key="mentor.id" :value="mentor.id">
                                    {{ mentor.name }}
                                </option>
                            </select>
                            <div v-if="form.errors.mentor_id" class="mt-1 text-xs text-red-500">{{ form.errors.mentor_id }}</div>
                        </label>
                    </div>
