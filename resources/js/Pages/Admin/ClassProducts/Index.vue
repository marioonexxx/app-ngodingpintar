...
const applyFilter = () => {
    router.get('/admin/class-products', {
...
const deleteProduct = async (product) => {
    ...
    router.delete(`/admin/class-products/${product.id}`, {
        preserveScroll: true,
        ...
        <section class="rounded-md bg-white p-6 shadow-sm shadow-sky-950/5">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    ...
                    <h1 class="mt-3 text-3xl font-black tracking-tight">Data Produk Kelas</h1>
                    <p class="mt-3 text-sm text-slate-500">Kelola katalog produk kelas, jadwal rutin, webinar, dan link pertemuan (meeting/akses grup).</p>
                </div>
        ...
        <section class="mt-5 rounded-md bg-white p-5 shadow-sm shadow-sky-950/5">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <Link href="/admin/class-products/create" class="rounded-md bg-[#0284c7] px-4 py-2 text-sm font-black text-white">Tambah</Link>
                </div>
                        ...
                        <tr v-for="product in products.data" :key="product.id" class="border-t border-slate-100">
                            <td class="px-4 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-1 text-xs font-semibold text-sky-600">{{ product.class_batches_count }} batch</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-4">
                                <span v-if="product.mentor" class="block w-max rounded bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-600">Mentor: {{ product.mentor.name }}</span>
                                <span v-else-if="product.vendor" class="block w-max rounded bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-600">Vendor: {{ product.vendor.name }}</span>
                            ...
                            <td class="px-4 py-4">
                                <div class="flex justify-end gap-2">
                                    <Link :href="`/admin/class-products/${product.id}/batches`" class="rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-600 hover:bg-emerald-100">Batch</Link>
                                    <Link :href="`/admin/class-products/${product.id}/edit`" class="rounded-full bg-sky-50 px-3 py-1.5 text-xs font-bold text-[#0284c7] hover:bg-sky-100">Edit</Link>
